...
import { PtItem, PtTask, PtComment, PtUser } from "../shared/models/domain";
import {
  PriorityEnum,
  StatusEnum,
  GenderEnum,
  ItemTypeEnum,
} from "../shared/models/domain/enums";
...
export function generatePTItem(index: number, users: PtUserWithAuth[]): PtItem {
  ...
  const typeStr = faker.random.arrayElement(
    Object.getOwnPropertyNames(ItemTypeEnum),
  );
  const type = typeStr as ItemTypeEnum;

  const priorityStr = faker.random.arrayElement(
    Object.getOwnPropertyNames(PriorityEnum),
  );
  const priority = priorityStr as PriorityEnum;

  const statusStr = faker.random.arrayElement(
    Object.getOwnPropertyNames(StatusEnum),
  );
...
export function generateUsersBase64Avatars(): PtUserWithAuth[] {
  const avatarsMenBase64 = getUserAvatars("src/images/avatars/base64/men.txt");
  const avatarsWomenBase64 = getUserAvatars(
    "src/images/avatars/base64/women.txt",
  );

  const users = _.times(NUM_USERS, (index: number) => {
    return generateUserBase64Avatar(
      index,
      avatarsMenBase64,
      avatarsWomenBase64,
    );
...
export function getMeUserBase64(index: number): PtUserWithAuth {
  const avatarMe = getUserAvatars("src/images/avatars/base64/me.txt")[0];
  const date = faker.date.past(1);
...
export function getMeUser(index: number): PtUserWithAuth {
  ...
  const userMe: PtUserWithAuth = {
    id: index + 1,
    fullName: "Alex Ziskind",
    avatar: "src/images/avatars/me/me.png",
    gender: GenderEnum.Male,
    dateCreated: date,
    dateModified: date,
...
export function generateUser(index: number): PtUserWithAuth {
  ...
  const avatar = `images/avatars/${
    genderBool ? "males" : "females"
  }/image-${index + 1}.png`;

  const authInfoA: PtUserAuthInfo = {
